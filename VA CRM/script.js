document.addEventListener('DOMContentLoaded', () => {
  ...
  const renderList = (listEl, items, template) => {
    listEl.innerHTML = '';
    items.forEach(i => {
  ...
  const renderEmails = (filter = '') => {
    const filtered = emails.filter(e =>
      e.subject.toLowerCase().includes(filter) || e.body.toLowerCase().includes(filter)
    );
    renderList(emailList, filtered, e =>
      `${e.subject} - ${e.body}` + (e.priority ? ' <span class="text-red-600 font-bold">[High]</span>' : '')
    );
  ...
  const renderEvents = (filter = '') => {
    const filtered = events.filter(ev =>
      ev.title.toLowerCase().includes(filter) || ev.date.includes(filter)
    );
  ...
  const renderNotes = (filter = '') => {
    const filtered = notes.filter(n =>
      n.client.toLowerCase().includes(filter) || n.note.toLowerCase().includes(filter)
    );
  ...
  const renderCalls = (filter = '') => {
    const filtered = calls.filter(c =>
      c.client.toLowerCase().includes(filter) || c.note.toLowerCase().includes(filter)
    );
